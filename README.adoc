...
class VirtualLayer {
    {field} -AklConfigurationProvider configurationProvider
...
class AklConfigurationProvider {
    {field} -FileInfo file
    {field} -AklConfiguration configuration
    {static} {method} +AklConfigurationProvider ReadFromFile(FileInfo file)
    {static} {method} +AklConfigurationProvider WithSaveDefault(FileInfo output)
    {method} +void SaveToFile()
}

AklConfigurationProvider - VirtualLayer
...
Internes Model für die De-/Serialization in Toml mit Tomlyn.
end note

AklConfigurationProvider -- AklConfiguration
